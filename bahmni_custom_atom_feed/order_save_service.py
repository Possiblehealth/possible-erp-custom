...
class OrderSaveService(osv.osv):
    ...
    def _create_sale_order_line_function(self, cr, uid, name, sale_order, order, context=None):
        ...
        if(stored_prod_ids):
            ...
            product_name_string = order['conceptName'].lower().split()
            common = set(product_name_string).intersection(set(liquid_drug_categ_names))
            ...
            if common:
                ...
                existing_products_with_same_name = self.pool.get('product.product').search(cr, uid,
                                                                                           [('name_template', '=ilike', order['conceptName'].strip()),
                                                                                            ('volume', '>=', 0)],
                                                                                           order='volume desc')
                # loop on all products, check whether there exists a product, whose volume is less that this product's volume, but greater than or equal to required volumn
                if existing_products_with_same_name[0] != prod_obj.id:
                    ...
                    comments = " ".join([str(actual_quantity)+'ML', str(order.get('quantityUnits', None))])
